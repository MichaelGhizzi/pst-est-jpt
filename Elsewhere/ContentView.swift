...
struct CardBadge<Leading: View>: View {
    let leading: Leading
    let text: String
    var showOffline: Bool = false

    init(text: String, showOffline: Bool = false, @ViewBuilder leading: () -> Leading) {
    ...
    var body: some View {
        HStack {
            VStack(alignment: .leading, spacing: 12) {
                ...
                HStack(spacing: 8) {
                    CardBadge(text: weather.sunrise) {
                        Image(systemName: "sunrise.fill")
                            .foregroundStyle(.yellow)
                    }
                    CardBadge(text: weather.sunset) {
